                                        ...
                                        <tbody className="divide-y divide-slate-200">
                                            {filteredLogs.map(log => (
                                                <tr key={log.id} className="transition-colors hover:bg-slate-50/80">
                                                    ...
                                                    <td className="px-4 py-3 text-slate-600 whitespace-nowrap text-sm">{log.clock_in ? new Date(log.clock_in).toLocaleString([], { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }) : '—'}</td>
                                                    <td className="px-4 py-3 text-slate-600 whitespace-nowrap text-sm">{log.clock_out ? new Date(log.clock_out).toLocaleString([], { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }) : '—'}</td>
